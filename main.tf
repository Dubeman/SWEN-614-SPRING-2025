...
resource "aws_instance" "wordpress_ec2" {
  ...
  user_data = templatefile("wp_rds_install.tpl", {
    ...
    db_host         = aws_db_instance.wordpress_db.endpoint # endpoint of the RDS instance, which is necessary for WordPress to connect to the MySQL database. Because there isn't an already running mysql server on the ec2. Instead the RDS is an internally managed service hosted by AWS which runs the MySQL server. One is directing WordPress to connect to the MySQL database hosted on the RDS instance.
  })
